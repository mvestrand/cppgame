...
namespace soba {
    ...
    void Command::parse_args(Repl const &repl, std::string const &cmd_str, std::vector<std::string> &args) const {
        repl.parse_args(cmd_str, args);
    }

    Repl::Repl() {}
    Repl::~Repl() {}
    bool Repl::readline_async() {
    ...
    void Repl::add_command(string const &name, shared_ptr<Command> command) {
    ...
    void Repl::free_command(string const &name) {
        auto it = commands.find(name);
        if (it != commands.end()) {
            commands.erase(it);
        } else {
            print_err("Failed to free unknown command \"" + name + "\"");
        }
    }
    ...
    int Repl::run(string const &cmd_str) {
        ...
        string cmd_name;
        if (!parse_command_name(cmd_str, cmd_name))
        ...
        // Process the arguments, then run the command
        vector<string> args;
        cmd->parse_args(*this, cmd_str, args);
        auto res = cmd->invoke(*this, args);
        return res;
    }

    bool Repl::parse_command_name(string const &cmd_str, string &name) const {
        regex words_regex("[^\\s]+");
        smatch sm;
